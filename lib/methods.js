...
const _outerJoin = (dataset1, dataset2, fieldList1, fieldList2) => {
    ...
    for (i = 0; i < dataset1.data.length; i++) {
        ...
        if (index[key]) {
            index[key].push(dataset1.data[i].concat(new Array(dataset2.fields.length)));
        } else {
            index[key] = [dataset1.data[i].concat(new Array(dataset2.fields.length))];
        }
    }
    for (i = 0; i < dataset2.data.length; i++) {
        key = [];
...
const sort = (dataset, sortStatement) => {
    ...
    var data = dataset.data.sort((a, b) => {
        for (var i = 0; i < sorts.length; i++) {
...
const aggregate = (dataset, aggregationList, groupList) => {
    ...
    var indicesList, record;

    for (i = 0; i < indices.length; i++) {
        indicesList = _splitList(indices[i]);
        record = [];
        for (j = 0; j < aggregations.length; j++) {
            if (!Number.isNaN(parseInt(aggregations[j][0]))) {
                record.push(indicesList[aggregations[j][0]]);
            } else {
